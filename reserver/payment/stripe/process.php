...
$affilation=false;
if (isset($_COOKIE['idaff'])){
  $stripeid=$_COOKIE['idaff'];
//   try{
// $bdd = new PDO('mysql:host=localhost;dbname=ma_base;charset=utf8', 'root', 'addafbaplm');
// // $bdd = new PDO('mysql:host=localhost;dbname=pornckys_tes;charset=utf8', 'pornckys_moi', 'test123');
// }
...
$req00 = $bdd->prepare('SELECT * FROM affiliation WHERE stripe_user_id = ?');
$req00->execute(array($stripeid));
if ( $req00->rowCount() > 0 ) {
...
echo httpPost("http://www.aventour.net/reserver/payment/stripe/processstripe.php",$params);
//die();
...
//$response = $paypal->request('DoExpressCheckoutPayment',$params);
 ...
 $affilation=false;
 ///ici faire plein de if pour stripe connect ou pas everifaint cookie et base de données
 
 $mprix=$_POST['prixreservation'];
 $mprix1= $mprix*(24/300);
 $mprix2=$mprix*(276/300);
...
//     for ($i=0; $i < strlen($mailHexVoyageur)-1; $i+=2){
//        $mailStringVoyageur .= chr(hexdec($mailHexVoyageur[$i].$mailHexVoyageur[$i+1]));
//     }
